...
\\
\\
Tabel \ref{tab:AsymInertia} summerizes the probit regression results. The only significant result ($p = \Sexpr{summary(hypo_Inertia_04_e)$coefficients[,4][2]}$) was for the end of the $R=0.4$ simulation, where $\beta = \Sexpr{hypo_Inertia_04_e$coefficients[2]} > 0$ indicating that inertia was more likely following a successful stragey. Thus the hypothesis was proven only for the end part of the $R = 0.4$.
...
\end{tabular}
\caption{Overview of the asymmetric inertia hypothesis test results}
...
Tabel \ref{tab:Reversion} summerizes the probit regression results. Non of the data-sets showed a $p < 0.01$ significance. The end of the $R=0.4$ simulation showed $p < 0.05$ significance with a result of $\beta = \Sexpr{hypo_Inversion_04_e$coefficients[2]} > 0$ indicating that reversion would be more likely after a successful adjustment. This would reject our hypothesis, but the significance level is too low.
...
\end{tabular}
\caption{Overview of the reversion hypothesis test results}
\label{tab:Reversion}
\end{center}
\end{table}
...
The asymmetric volatility hypothesis states that the variance of adjustments following payoff decrease, e.g \emph{failure}, is larger than the variance of adjustments following \emph{successful} strategies.  The hypothesis was tested using the Levene's test. Tabel \ref{tab:AsymVolatility} summarizes the results of the hypothesis testing. The only significant result ($p =  \Sexpr{hypo_Asym_Volatility_04_e$p.value} $) is for the end of the $R=0.4$ simulation, indicating that the hypothesis is true.
...
\end{tabular}
\caption{Overview of the asymmetric volatility hypothesis test results}
...
where $Success(i,t-1)$ is defined as above. Table \ref{tab:AsymBreadth} summerizes the test results. The only case for which a significant ($p = \Sexpr{ summary(hypo_Asym_Breadth_04_e)$coefficients[,4][2]}$) result is found is the end of the $R=0.4$ simulation, where $\beta = \Sexpr{hypo_Asym_Breadth_04_e$coefficients[2]} < 0$, indicating that hypothesis is true (success lowers the size of the adjustments).
...
\end{tabular}
\caption{Overview of the asymmetric breadth hypothesis test results}
...
such that cases where $c_i^{t-1} - c_i^{t-2} \ge 0$ and $c_i^{t-1} - c_i^{t-2} < 0$ are treated separetly. Also cases where there was inertia during the previous time step are not considered for the statistics. Table \ref{tab:DirectionalBias} summarizes the test results. Only the beginning of the $R=0.4$ simulation gives significalt results. But these results indicate that for both $c_i^{t-1} - c_i^{t-2} \ge 0$ and $c_i^{t-1} - c_i^{t-2} < 0$ adjustments following successful adjustments are \emph{larger}. This verifise half of the hypothesis but falsifies the other half.
...
\end{tabular}
\caption{Overview of the directional bias hypothesis test results}
